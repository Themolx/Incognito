...
def replace_clip_name(item):
    """Rename the timeline clip directly."""
    try:
        ...
        print(f"Renaming clip: {clip_name} -> {product_name}")

        try:
            # Set the clip name directly in the timeline
            item.SetProperty("Clip Name", product_name)
            print(f"Successfully renamed clip to: {product_name}")
            return True
...
def update_timeline():
    ...
    video_tracks = timeline.GetItemListInTrack('video')
    if not video_tracks:
        print("No video tracks found in timeline")
        return
    ...
    # Process each track
    for track_items in video_tracks:
        if not track_items:
            continue
    ...
    print(f"Processed {success_count} of {total_count} clips successfully")
